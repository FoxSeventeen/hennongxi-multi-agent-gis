...
本项目不是通用遥感平台。当前只支持批准的神农溪场景、两期影像和固定分析链；除已批准的高德
普通道路上下文地图外，临时增加第二地区、任意上传、任意模型工具、其他高德图层或公开部署都
属于范围变更。
...
| 组件 | 宿主端口 | 核心职责 | 不负责的事情 |
| --- | ---: | --- | --- |
| Web | 3000 | 中文任务输入、就绪状态、Agent 时间线、高德上下文/MapLibre 成果地图、指标、重试、报告下载 | 浏览器只接收 JS Key、Web 服务端只接收安全密钥；不持有 Master Web 服务 Key，不计算 GIS，不直连私有 Agent |
...
sequenceDiagram
    ...
    Web->>AMapJS: 无成果时加载普通道路图、转换固定中心点
...
高德有两个彼此隔离的可选边界：Master 的 Web 服务调用只生成研究区位置证据；浏览器 JS API
只在没有合法 publication 时提供普通道路上下文。两者都不影响 G2 已批准的流域边界、日期、
红光/近红外数据、阈值和计算公式。Web 服务未配置、超时或限流时，规范内任务记录 `DEGRADED`
后继续；JS API 未配置或加载失败时，页面回退离线占位图，任务状态不变。大模型调用失败时，
系统可以使用明确标记的内置恢复计划继续固定链；页面和报告必须区分“真实大模型规划”和
“恢复规划”。
...
- 浏览器调用 Master 的任务/健康/SSE API、Publisher 的只读资源 API、批准的高德官方 JS/地图
  域名和 Web 的固定同源 `/_AMapService` 代理；没有其他第三方边界。
...
- 大模型 Key 与高德 Web 服务 Key 只存在于 Master；高德 `securityJsCode` 只存在于 Web 服务端；
  专用 Web端（JS API）Key 按官方机制对浏览器可见，必须受域名限制并可轮换。
- 同源代理固定目标主机、路径、方法、超时、响应大小和并发，拒绝客户端提供安全密钥、上游、
  重定向、私网地址或任意请求头，不能成为开放代理或 SSRF 通道。
- 高德请求不包含完整用户提示、任务 ID、流域几何、影像或成果；浏览器只临时转换一个固定
  WGS84 中心点，不缓存或持久化脚本、瓦片、道路、转换结果和原始响应。
...
| 路径 | 内容 |
| --- | --- |
| `apps/web/` | React/TypeScript、高德上下文地图、同源安全代理与 MapLibre 成果前端 |
...
4. **高德能力与成果地图隔离。** Master 只做可选地名校验；浏览器只做无成果时的实时普通道路
   位置参考。合法成果始终由 MapLibre 独占显示，GCJ-02 不与 WGS84 遥感图层叠加，第三方故障
   也不会阻断任务链。
