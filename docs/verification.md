...
| 高德 Web 服务 | `<启用/未启用；专用 Key 已轮换：是/否>` |
| 高德 Web端（JS API） | `<启用/未启用；专用 Key/安全密钥已轮换：是/否>` |
...
### 4.3 高德 Web端（JS API）上下文地图（计划展示时）

这一项使用独立 Web端 Key 与配套安全密钥，不由 4.2 的 Web 服务 Key 替代。启动/重建 Web 后，
在浏览器中完成一次显式在线检查；只记录以下脱敏字段，不保存高德地图截图、转换坐标、原始响应、
Key 或安全密钥：
...
| 检查时间 | `<时间>` |
| 无任务普通道路图 | `<可见/不可见>` |
| 任务期间同一实例/短 ID | `<通过/不通过>` |
| 中心区域 | `<神农溪区域正确/不正确；不写转换坐标>` |
| 官方 Logo/版权 | `<可见/被遮挡>` |
| 合法成果后 MapLibre 切换 | `<通过/不通过>` |
| 浏览器控制台 | `<干净/脱敏错误码>` |
| 高德域名集合 | `<只写域名，不写路径或查询串>` |
| 同源代理 | `<仅 /_AMapService；安全密钥未出现在网络请求>` |

项目所有者已取消目标机器主动断开/恢复真实外网的人工验收步骤。T29 以本表的真实在线结果，加上
组件测试和隔离 Playwright 网络失败旅程中的 5 秒回退、任务完成、MapLibre 与报告证据验收。
不要填写或声称不存在的真实断网时间、任务 ID 或恢复结果；若现场偶发真实网络故障，可按排障
文档记录为附加证据，但它不再是 T29/T28 的必需门禁。

## 5. 自动化基线

| 运行 | 准备 | 通过条件 | 实际结果 |
| --- | --- | --- | --- |
| 冷启动 E2E | 仅清空 `hennongxi-e2e` 专用 volumes | Playwright 5/5 | `<待填>` |
| 温缓存 E2E | 保留同一专用 volumes 再运行 | Playwright 5/5 | `<待填>` |
...
| 创建前上下文地图 | `<高德普通道路/离线占位；状态文案>` |
| 执行中上下文地图 | `<同一实例/离线占位；任务短 ID>` |
| 成果地图切换 | `<高德已销毁；MapLibre 三类图层可用>` |
| 五个阶段 | `<全部完成；分别记录耗时>` |
...
截图最少包含：首页就绪、同一 ID 的五 Agent 时间线、三类 MapLibre 成果地图之一与完整流域、
面积/质量、PDF 正文、刷新恢复。截图放在 `tmp/t28/` 或仓库外；检查其中没有 Key、供应商 URL
或个人信息。不要保存真实高德道路地图截图；首页截图应在离线占位状态拍摄或裁掉地图区域。
...
分别记录两类高德凭据未配置；真实外网断开已由项目所有者豁免，不要合并或补写为通过：

| 场景 | Master 位置状态 | 浏览器上下文 | 规划来源 | 终态 | 四栅格/统计/质量与在线一致 |
| --- | --- | --- | --- | --- | --- |
| Web 服务 Key 置空 | `<DEGRADED>` | `<高德/离线占位；按实际配置>` | `<真实 LLM>` | `<COMPLETED>` | `<是>` |
| Web端 Key/安全密钥置空 | `<VERIFIED/DEGRADED；按实际配置>` | `<离线占位；无高德请求>` | `<真实 LLM>` | `<COMPLETED>` | `<是>` |
| 主机外网断开（已豁免） | `N/A（未执行）` | `N/A（未执行）` | `N/A（未执行）` | `N/A（未执行）` | `<引用自动化回退证据>` |

若现场偶发外网故障，恢复后再次执行健康检查、真实 LLM 冒烟和（启用时）高德冒烟，可把恢复
时间记为附加证据。自动化离线链不能替代第 4 节的真实 LLM 成功证据。
...
- [ ] Web 服务 Key、Web端 Key 和安全密钥互不混用；只有 Web端 Key 在浏览器可见。
- [ ] 安全密钥未进入 HTML、JavaScript bundle、source map、公共响应或浏览器请求。
- [ ] 原始/缓存影像、生成成果、PDF 和测试产物未进入 Git。
- [ ] 高德没有替代 WGS84 流域、Sentinel-2 影像或批准日期。
- [ ] 未缓存/持久化高德脚本、道路、瓦片、转换结果、原始响应或真实地图截图。
- [ ] 只使用 `docker compose down` 非破坏性停止，命名卷仍存在。
- [ ] `git status --short` 只含演示前已有的预期文件。
- [ ] 两类高德 Key/安全密钥在交付或曝光后已轮换，真实大模型 Key 的保管人明确。
