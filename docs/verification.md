...
随后断开真实高德网络或目标机外网，确认 5 秒加载边界内出现“地图已就位”，创建任务、时间线、
成果、MapLibre 和报告仍可完成。恢复网络后刷新页面，重新记录恢复时间。只有在线成功在前、断网
回退在后，才能把 T29 的真实浏览器冒烟标为通过。
...
| 主机外网断开 | `<DEGRADED>` | `<5 秒边界内离线占位>` | `<BUILTIN_RECOVERY 或实际值>` | `<COMPLETED>` | `<是>` |
