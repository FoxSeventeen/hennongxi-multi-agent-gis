...
本文档供实际答辩操作员使用。目标是让演示可重复、可解释、失败时不误操作，并留下 T28 所需的真实证据。

- 首次安装请先完成 [`setup.md`](setup.md)
- 每次演练的记录填写到 [`verification.md`](verification.md) 的本地副本

## 1. 演示原则

1. **使用真实数据和服务**：正式演示使用真实缓存栅格和真实大模型；高德 Web 服务校验与浏览器地图都是可选的位置增强，不是主链依赖
2. **不在现场做危险操作**：不在答辩现场下载影像、升级依赖、构建新缓存或修改代码
3. **保护敏感信息**：不在投屏终端显示 `.env`、供应商控制台、完整日志或任何 Key
4. **保留失败证据**：不删除失败任务；失败记录是系统诚实性的证据，修复依赖后从安全检查点重试
5. **安全停止**：不执行 `docker compose down -v`，普通停止始终使用 `docker compose down`
...
如果只有一人，提前把健康检查终端、浏览器首页和本手册分别放在固定窗口，不要在投屏时搜索命令。
...
**第二条（高德 Web 服务）**：只验证 Master 的高德 Web 服务地名校验，并且只在答辩计划展示该能力时要求退出 0；否则记录未配置/降级状态，**不要因此改变遥感数据**。

**浏览器的高德地图**：使用另一个 Web 端（JS API）Key 与安全密钥。如计划展示，打开首页确认地图和官方 Logo/版权可见，但**不要保存地图截图**。

把允许的脱敏字段抄到本地验收记录，不保存终端中其他输出。
...
项目所有者已取消目标机器主动断开/恢复真实外网的人工验收。7.1 与 7.2 仍可用于配置排障；
7.3 只描述意外网络故障时的预期，不属于 T29/T28 必做步骤。自动化网络失败旅程仍是发布回归
门禁，不能因取消人工演练而删除。
...
不要为了验收主动断开目标机外网，也不要用修改 `/etc/hosts`、防火墙或容器网络的临时命令制造
证据。若运行现场意外失去真实外网，但 OrbStack/Docker 和本地 Compose 网络仍在运行，则预期：
...
网络恢复后执行健康检查、两个真实上游冒烟，并刷新首页确认高德上下文地图恢复。偶发故障记录
只能作为附加证据，不替代至少一次真实大模型成功证据，也不能被补写成计划内的断网验收。
