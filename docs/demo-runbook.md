...
1. 正式演示使用真实缓存栅格和真实大模型；高德是可选位置增强，不是主链依赖。
...
第一条必须退出 0。第二条只在答辩计划展示在线高德校验时要求退出 0；否则记录未配置/降级状态，
不要因此改变遥感数据。把脱敏 JSON 字段抄到本地验收记录，不保存终端中其他输出。

### 3.4 自动化基线

E2E 使用独立的 `hennongxi-e2e` Compose 项目、确定性小栅格、假大模型和假位置服务，不消耗真实
额度，也不会覆盖日常项目的命名卷。
...
预期 Playwright `4 passed`。保留 E2E volumes，再执行一次温缓存复跑：

```bash
./tests/e2e/run.sh
```

仍应为 `4 passed`。失败诊断只保存在 Git 忽略的 `tests/e2e/test-results/` 和
...
| 时间 | 页面动作 | 讲解重点 |
| --- | --- | --- |
| 0:00–0:40 | 首页与就绪卡片 | 这是五 Agent 的真实容器系统，内部服务不公开端口 |
...
3. 任务应最终在发布阶段显示 `FAILED`，页面必须没有质量“通过”冒充的完整结果，也不能提供新
   PDF；记录任务 ID、attempt 1、失败 Agent、错误码和页面截图；
4. 恢复 Publisher：
...
任务进入终态后恢复外网，执行健康检查和两个真实上游冒烟。该步骤证明离线主链，但不替代至少
一次真实大模型成功证据。
