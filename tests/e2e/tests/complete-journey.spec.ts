...
test("确定性 Compose 主旅程完成地图、指标、报告与刷新恢复", async ({
  ...
  await expect(page.getByText("等待创建任务；这里不表示遥感分析成果。")).toBeVisible();
  ...
  const amapLifecycle = await page.evaluate(
    () =>
      (
        window as Window & {
          __HENNONXI_E2E_AMAP__?: {
            conversions: number;
            lastConversion: { point: number[]; type: string } | null;
            mapsCreated: number;
            mapsDestroyed: number;
            securityServiceHost: string | null;
          };
        }
      ).__HENNONXI_E2E_AMAP__,
  );
  expect(amapLifecycle).toEqual({
    conversions: 1,
    lastConversion: { point: [110.299073, 31.262497], type: "gps" },
    mapsCreated: 1,
    mapsDestroyed: 1,
    securityServiceHost: "http://127.0.0.1:3000/_AMapService",
  });
...
test("Publisher 强制失败后界面保留证据并从安全检查点重试完成", async ({
  ...
  await expect(
    page.getByText(
      `任务 ${(taskId ?? "missing").slice(0, 8)} 正在生成分析成果；高德仅提供位置参考。`,
    ),
  ).toBeVisible();
