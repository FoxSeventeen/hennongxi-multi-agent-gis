...
用户已于 2026-07-20 回复 `批准 G2 反射率修正`。按本文件限定的范围实施和验收。

## 实施与验收结果

- 六个不可变源文件均通过原 SHA-256 核验，未发生重新下载；
- 2019-08-19 两个缓存文件保持原字节数和 SHA-256 不变；
- 2024-08-12 红光缓存更新为 43,096,434 字节，SHA-256 为
  `c14aa6d4606e083cf04cf489f117a078fe41a711140aae510e69196c7d6f4dc9`；
- 2024-08-12 近红外缓存更新为 42,100,556 字节，SHA-256 为
  `84f645c9295efc1795450e8619ed00473c6c45f022eea5844deb2d13ee6ba42e`；
- 真实 NDVI 后时相范围为 `[-0.989744, 0.999684]`，差值范围为
  `[-1.538007, 1.211899]`；
- 真实 Data → Analysis → Quality 私网测试通过：覆盖率 `1.0000`、有效像元率
  `0.9312`、完整性 `5/5`、Analysis 耗时 `3319 ms`，最终结论为 `PASS`；
- 所有既定质量阈值保持不变。
