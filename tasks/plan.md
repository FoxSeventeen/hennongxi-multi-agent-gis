...
- [ ] 使用专用可轮换 Web端（JS API）Key 完成一次显式真实浏览器冒烟，确认普通道路地图、
  正确区域、官方标识和干净控制台；随后阻断高德网络，确认 5 秒内回退且任务主链不受影响。
  证据不保存高德地图截图、Key、安全密钥、原始响应或转换结果。

**自动化结果（2026-07-22）：**Web 组件与集成共 73 项测试通过，TypeScript、ESLint 和生产构建
...
安全配置扫描命中数为 0。真实在线冒烟尚未执行，因此 T29 与其最后一项保持未完成。
