...
- [ ] 在上述在线成功证据之后阻断真实高德网络，确认 5 秒内回退且任务主链不受影响；恢复网络
  后刷新并确认高德地图恢复。两项证据都不得保存高德地图截图、Key、安全密钥、完整 URL、
  原始响应或转换结果。
...
地图链，不作为 T28 真实大模型证据。真实断网及恢复检查仍未执行，因此 T29 保持未完成。

README、架构、安装、开发、答辩、验收、排障和 E2E 说明已同步为中文双边界口径，明确申请、
配置、轮换、在线检查、证据限制和离线恢复步骤。

**依赖：**T21-T22、T27；门禁 G6。T28 在 T29 完成前不得开始最终真实演练或标记完成。
