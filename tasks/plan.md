...
- 2026-07-22 变更已批准：按 `tasks/approvals/G6-amap-context-map.md` 增加高德普通道路上下文
  地图、任务期间持续显示、成果态切回 MapLibre 和离线占位回退；用户以
  “批准 G6 高德上下文地图”批准。该补充任务编号为 T29，并先于仍未完成的 T28 演练执行。
...
10. G6 新增的 Web JS API Key 与第 9 项所指 Master Web 服务 Key 完全分离；浏览器上下文地图
    只负责位置参考，不进入任务就绪门禁、分析输入、成果发布或 PDF，并可随时回退离线占位图。
...
G6 只补充 G5 中“浏览器不使用高德底图”的限制：没有合法 publication 时，Web 可以使用高德
JS API 2.0 默认普通道路地图作为位置参考；合法成果仍由 MapLibre 独占显示。浏览器只发送一个
固定 WGS84 中心点做临时 `gps` 转换，不发送流域几何、任务标识、提示、分析、质量或报告。
Web JS API Key 与 Master Web 服务 Key 分离，安全密钥只由固定同源代理持有；任何未配置、超时、
鉴权或网络故障都回退现有离线占位图，不改变就绪、任务状态或成果门禁。完整范围、禁止项和
验收证据以 `tasks/approvals/G6-amap-context-map.md` 为准。
              ...
              -> T19 Web shell -> T20 timeline -> T21 map -> T22 results/retry UI
                  -> T23 AMap adapter -> T24 study-area grounding
                      -> T25 contract/integration suite -> T26 Compose/E2E
                          -> T27 hardening -> T29 AMap context map -> T28 rehearsal/handoff
...
| 10 | T26-T27、T29、T28 | Compose 浏览器旅程、加固、上下文地图、真实演练与可复现交付 |
...
- [x] 一个命令可构建/启动完整栈并运行中文答辩主旅程，无需人工后端干预。
- [x] 浏览器断言完整流域、三个计算图层、必需指标、匹配报告/任务 ID、刷新恢复、失败/重试，
  以及高德“验证通过/已降级”两种真实文案。
- [x] 失败截图、trace 和日志可供诊断；成功生成物保持 Git 忽略，测试不访问真实高德服务。

**验证：**

- [x] 从仓库根目录执行 `./tests/e2e/run.sh`；该脚本等价替代规格中的 E2E 容器命令，并显式
  重建 Master、Web、PostGIS 和 Playwright 镜像、等待完整栈就绪后运行验收。
- [x] 在目标 Apple Silicon 机器上通过全新 volume 和温缓存复跑。

**依赖：**T04-T05、T19-T22、T25；门禁 G4。

**实际修改文件：**`tests/e2e/` 下的 Playwright 镜像、锁文件、配置、中文旅程、诊断夹具、
确定性种子/假上游、测试专用 Master 与 Publisher 失败代理、Compose 覆盖层、一键脚本和中文
运行说明；基础 Compose 增加仅在 `e2e` profile 下启用的 Runner；Master 运行时只增加显式的
研究区校验依赖注入点，生产默认行为不变。

**完成证据（2026-07-21）：**`./tests/e2e/run.sh` 在清空 `hennongxi-e2e` 的全部 volumes 后
从零构建并通过 4/4 Chromium 旅程；保留 PostGIS、Redis、缓存和成果 volumes 后用同一命令
温缓存复跑再次通过 4/4。旅程覆盖完整流域、前/后期 NDVI 与差值、四项面积统计、四项质量
指标、任务绑定 PDF、刷新恢复、在线位置“验证通过/已降级”、Publisher 首次强制失败、界面
安全重试、第 2 次执行复用三个安全检查点，以及第 1 次失败事件的耐久重放。最终定向门禁为
27 项 Python/契约/拓扑测试、Ruff、格式、3 个 Python 文件的 Mypy、E2E TypeScript、Compose
配置和 pnpm 高危审计全部通过；审计报告无已知漏洞。测试只使用确定性小型 GIS 数据、假大
模型和测试专用位置校验器，不访问真实高德；截图、trace、HTML、浏览器日志、Compose 日志和
成功生成物均位于 Git 忽略目录。

**实施提交：**`8a52468`、`c8b8c52`、`206c41f`、`bd544b6`、`81d15f9`。

**工作量：**中等。

#### 检查点 H2：可接受用户流程

- [x] T22-T26 的验证可从仓库根目录以文档命令运行。
- [ ] 规格的 12 项成功标准均有自动化证据或具名人工演练检查。
- [x] 新鲜与温缓存运行均不把密钥、原始数据、高德响应或生成成果加入 Git。
...
- [x] 负向测试覆盖任务输入、LLM/高德输出、内部载荷、成果/瓦片路径、跨任务访问、超时、
  重定向、响应上限和密钥脱敏。
- [x] 结构化日志可跨容器追踪一个任务/尝试，但不含凭据、敏感原始提示、私有数据 URL 或
  高德完整请求/响应。
- [x] 健康/就绪、容器限制和错误消息可区分用户错误、必需依赖故障与高德可选增强降级；人工
  复核确认没有抓取、缓存、镜像或持久化高德相关内容。

**验证：**

- [x] 加固后全量 lint、类型、单元、契约、集成和浏览器测试通过。
- [x] 人工密钥扫描、跨任务/目录穿越测试和高德数据留存检查均无泄漏或越权。

**依赖：**T26。

**预计修改文件：**审查发现的聚焦服务/Web 模块、安全测试和运行文档；无关发现分别提交。

**实际结果：**Master 的大模型、高德和内部 Agent HTTP 客户端统一强制 `identity` 编码，并在
读取响应体前拒绝非法内容编码、非规范长度及已声明超限响应；未知长度响应继续按解码后字节数
流式限额。Data Agent 现在强制请求头与命令体使用同一 UUID 关联标识，并输出包含任务、步骤、
尝试和关联标识的开始、完成、预期失败及意外失败结构化日志，异常详情、请求体、本地文件/
成果路径和数据源地址均不进入日志或 500 响应。既有健康/就绪和研究区事件继续分别表达
必需依赖故障与高德 `VERIFIED/DEGRADED` 可选增强状态，无需修改公开契约。

基础 Compose 为全部运行容器设置 CPU、内存和 PID 硬上限；实测代理为 2 核/2 GiB/256 PID，
Web 为 0.75 核/1 GiB/128 PID，PostGIS 为 1 核/1 GiB/256 PID，Redis 为 0.5 核/512 MiB/128 PID，
Playwright 为 2 核/3 GiB/512 PID。E2E 种子、迁移和支持服务也有独立上限。合并模型测试发现并
修复了 Compose 列表追加导致的 E2E 公共网络残留；覆盖栈中的全部服务现在只共享 internal
`private` 网络，真实 4/4 Playwright 旅程仍可访问 Web 和内部 API。Web 的初始 512 MiB 上限会
使 Node 24 在 ESLint 中稳定耗尽约 256 MiB 堆，故按失败证据校准为 1 GiB，并重新通过测试、
Lint、类型和生产构建。

**实际修改文件：**`services/master/src/hennongxi_master/http_safety.py` 及三个上游客户端与测试、
`services/data_agent/src/hennongxi_data_agent/main.py` 及测试、`docker-compose.yml`、
`tests/e2e/compose.yml`、Compose 拓扑测试、`tasks/plan.md`、`tasks/todo.md`。

**完成证据（2026-07-22）：**共收集 511 项 Python 测试；全新隔离 PostGIS/Redis 与真实私网
Agent 中 490 项通过、5 项按环境设计跳过，宿主机 16 项最终 Compose 模型测试通过，即 506 项
通过、5 项跳过。安全负向专项 234/234、Web 46/46、Playwright 4/4 均通过；全仓 Ruff、60 个
生产源码文件的严格 Mypy、TypeScript、ESLint、OpenAPI 类型漂移检查、Vite 生产构建、基础及
E2E Compose 配置均通过。Web 与 E2E 两个 pnpm 锁文件的高危审计均无已知漏洞。

人工检查确认 `.env` 被忽略且未跟踪，当前 Git 与全部历史未发现疑似真实凭据；当前 Compose
日志、PostGIS、Redis、数据缓存卷、成果卷和质量报告卷中的高德原始 URL/字段特征命中数均为
0。高德协议解析与调用仍仅存在于 Master 适配器/冒烟入口及测试，不抓取或保存高德底图、边界、
POI、完整请求或原始响应。最终五轴及三轮聚焦对抗复核未发现未关闭的 T27 阻断项。

**实施提交：**`eea4dd9`、`0aa3d0e`、`a7b6444`、`176b23b`。

**工作量：**中等。

#### 补充任务 29：接入高德上下文地图并保留 MapLibre 成果地图

**说明：**按已批准 G6，在没有合法发布成果时使用高德 JS API 2.0 默认普通道路地图提供位置
参考；任务执行期间保持同一地图实例，合法成果到达后销毁高德并复用现有 MapLibre 成果地图。
高德未配置、超时、鉴权或网络失败时，稳定回退现有离线占位图，不改变任务、Agent、质量、
发布或重试语义。T29 是 G6 后批准的补充编号，但执行顺序先于仍未完成的 T28。

**验收标准：**

- [x] `AMAP_WEB_SERVICE_KEY`、`VITE_AMAP_JS_API_KEY` 与 `AMAP_JS_API_SECURITY_CODE` 三套配置
  互不混用；安全密钥只在 Web 服务端通过固定 `/_AMapService/*` 只读代理使用，真实值不进入
  Git、浏览器 bundle、日志、存储或报告。
- [x] 安全代理只连接批准的高德 HTTPS 主机和路径，拒绝用户指定上游、重定向、非法方法、
  超时、超限响应与超额并发；错误保持脱敏，不能成为开放代理或 SSRF 通道。
- [x] 无任务、任务执行中和无成果失败态显示普通道路地图与准确状态文案；固定 WGS84 中心点
  仅经 `AMap.convertFrom("gps")` 临时转换，不发送完整流域、任务标识、提示或成果数据。
- [x] 高德未配置、5 秒超时、Loader/转换/鉴权/网络失败和卸载竞态均回退现有离线占位图；
  用户仍可创建、观察、刷新和重试任务，没有白屏、无限重试或未处理 Promise。
- [x] 合法 publication 到达后高德实例、事件和容器引用已销毁，既有 MapLibre 三类 NDVI、
  流域边界、图例、日期、attribution、瓦片错误和重载行为无回归；非法 publication 不显示
  高德来掩盖错误。

**验证：**

- [x] 先失败后通过的 Vitest 覆盖代理策略、未配置、成功、任务期间复用、5 秒超时、失败回退、
  卸载竞态、成果切换和既有 MapLibre 行为；TypeScript、ESLint 与 Vite 生产构建通过。
- [x] 默认 Playwright E2E 使用假 Loader 或网络拦截，冷/温缓存旅程均不访问真实高德；Compose
  配置、依赖审计、凭据扫描和高德数据无留存检查通过。
- [x] 使用专用可轮换 Web端（JS API）Key 完成一次显式真实在线浏览器冒烟，确认普通道路地图、
  固定神农溪中心、官方标识、任务期间复用、成果切换和干净控制台。
- [x] 验收例外：项目所有者取消目标机器主动断开/恢复真实外网的人工步骤；以既有组件测试和
  隔离 Playwright 网络失败旅程证明 5 秒回退与任务主链不受影响，不虚构真实断网证据。

**自动化结果（2026-07-22）：**Web 组件与集成共 77 项测试通过，TypeScript、ESLint 和生产构建
通过；E2E Python/Compose 契约 12 项通过。隔离 Playwright 以确定性占位凭据拦截高德 Loader，
验证固定中心点 `gps` 转换、无任务位置参考、失败态任务短 ID、成果到达后的实例销毁与 MapLibre
切换；高德网络被阻断时在配置的 5 秒超时后回退离线占位图，任务仍完整完成。冷、温缓存两轮均
为 5/5 通过，浏览器没有未处理异常。已跟踪文件、生产 bundle 和成功 E2E 产物的凭据/服务端
安全配置扫描命中数为 0。

**真实在线结果（2026-07-22）：**修复并推送 `b6e54c4` 后，使用本地 Git 忽略配置完成一次不留
截图的真实浏览器检查。14:57:35 CST 创建任务
`1d1cc8de-85b8-4f46-b6f2-8d216224e400`，38,389 毫秒后状态为 `COMPLETED`。空闲态和任务态均
只有一个高德 Canvas，官方 Logo/版权可见，加载状态正常结束且离线占位未出现；任务文案显示
短 ID。合法成果到达后高德实例和标识消失，MapLibre 三类图层、完整流域、2019-08-19、
2024-08-12、质量结论和 PDF 入口均出现。控制台 warning/error 均为 0；高德侧域名集合为
`webapi.amap.com`、`jsapi.amap.com`、`o4.amap.com` 与同源代理，只有一次固定中心转换，浏览器
请求中没有 `jscode`、任务 ID 或提示词。本次任务使用 `BUILTIN_RECOVERY` 规划，只证明 T29
地图链，不作为 T28 真实大模型证据。

**验收例外（2026-07-22）：**项目所有者以“断网测试就算了，因为实际环境不可能没有网络”取消
真实断网及恢复检查。该项没有执行，也不记录为通过；T29 依据真实在线冒烟、77 项 Web 测试、
隔离 E2E 网络失败旅程和既有安全/无留存检查完成。运行时离线回退规则及自动化回归门禁不变。

README、架构、安装、开发、答辩、验收、排障和 E2E 说明已同步为中文双边界口径，明确申请、
配置、轮换、在线检查、证据限制和离线恢复步骤。

**状态：**T29 已完成，T28 的前置依赖已解除。

**依赖：**T21-T22、T27；门禁 G6。

**预计修改文件：**`apps/web/`、`.env.example`、`docker-compose.yml`、`tests/e2e/`、中文安装/
演示/验证/排障文档、`tasks/plan.md` 与 `tasks/todo.md`。

**工作量：**中等。

#### 任务 28：演练真实答辩并完成交付文档

**说明：**在目标机器使用批准的缓存真实数据、真实 LLM 和显式高德在线冒烟完成一次记录化
全栈演练；记录配置、检查、演示、限制、恢复和非破坏性停止。真实断网复跑已由项目所有者取消，
但自动化高德失败回退证据仍须保持通过。
...
- [ ] 一次真实 LLM/真实栅格 UI 运行满足 12 项成功标准，并记录任务 ID、耗时、校验和、
  截图/日志引用、位置校验状态和中文报告。
- [ ] 页面刷新与强制失败/重试成功；核对自动化网络失败旅程证明规范内任务仍可用缓存数据完成，
  答辩时不需要下载影像，也不把高德作为必需依赖。
- [ ] README/运维/演示文档让新操作员能安全配置、预检、启动、验证、演示、停止和排障，
  并明确 Key 轮换、高德额度/状态码和 GCJ-02/WGS84 限制。

**验证：**

- [ ] 在目标机器执行 `docs/demo-runbook.md` 的必需命令，包括冷启动、就绪、冒烟/E2E、真实
  工作流、重试和非破坏性停止；不主动执行已取消的真实断网步骤。
- [ ] `git status --short` 只含预期源码/文档；原始数据、`.env*`、高德响应、输出、报告和
  日志保持忽略。

**依赖：**T05、T14、T27、T29；门禁 G2-G6；需要有效 LLM 配置、可轮换的高德 Web 服务 Key、
Web端（JS API）Key、安全密钥和运行中的容器后端。

**预计修改文件：**`README.md`、`docs/setup.md`、`docs/demo-runbook.md`、
`docs/verification.md`。

**文档交付进度（2026-07-22）：**已完成中文 `README.md`、架构说明、目标机安装、答辩操作、
T28 验收证据模板和非破坏性排障文档，并按当前 Compose、数据清单、冒烟入口、E2E 脚本和安全
边界完成静态核对。文档明确区分自动化基线、真实大模型/栅格/高德证据与离线恢复计划，且不会
把未发生的目标机演练写成完成。仍需接手操作员在目标机器逐项执行、填写任务 ID/耗时/校验和并
由第二人复核；因此 T28 验收项和最终检查点继续保持未完成。
...
| 风险 | 影响 | 缓解措施 / 最早验证任务 |
| --- | --- | --- |
| 高德服务超时、限流或现场无网 | 高 | T23 严格超时/状态码，T24 可观测降级，T29 组件与隔离 E2E 网络失败旅程持续验证 5 秒回退；不设为核心就绪阻塞项 |
| 高德 Key 或安全密钥泄漏到错误边界、日志或 Git | 高 | Master Web 服务 Key 仅注入 Master；Web JS Key 仅按官方机制暴露给浏览器；安全密钥只在 T29 同源代理；忽略 `.env*`、脱敏测试、凭据扫描并在交付前轮换 |
| GCJ-02 与 WGS84 混用导致错位 | 高 | 不替换 MapLibre、不直接叠加高德坐标；G2 几何与分析契约不变，T27 人工复核 |
| 高德数据留存或展示超出官方授权 | 高 | 不缓存/持久化/下载原始响应、边界、POI、底图或转换结果；上下文地图仅实时展示并保留官方标识，T29 做无留存检查 |
...
| G6 补充范围导致最终演练延期 | 高 | T29 按代理、组件、状态接入和验收切片提交；只加入普通道路上下文地图，不加入卫星、路线、天气、POI 或第二 GIS 场景 |
...
6. **G6 已批准：**按 `tasks/approvals/G6-amap-context-map.md` 接入高德普通道路上下文地图、
   固定同源安全代理、任务期间持续显示、成果态切回 MapLibre 和离线占位回退；不改变 G2
   数据、分析、质量、Publisher 成果、业务 OpenAPI 或 PDF。

不得通过静默扩大范围来处理其他开放问题。若要增加第二个 GIS 场景、G6 以外的高德图层或
能力、提供商专用业务 SDK、新基础设施服务、数据库迁移或公开工作流路由，必须停止实施并
请求审批。
